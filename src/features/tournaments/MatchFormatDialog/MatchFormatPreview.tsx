...
/**
 * Lightweight preview generator for formats.
 * Uses registered count to create placeholder teams "Team 1..N".
 * For real data, supply actual participant list instead of placeholders.
 */
...
function buildKnockoutPairs(teams: string[], bracketSize: number, seedMethod: string | null) {
    // simple seeding: if ranking then keep order, if random then shuffle, manual leave order
    let arr = [...teams];
    ...
    // fill with BYE placeholders until bracketSize
    const byes = bracketSize - arr.length;
    for (let i = 0; i < byes; i++) arr.push("BYE");
    // pair top vs bottom for simple seeding
...
export default function MatchFormatPreview({ open, onOpenChange, category, format }: Props) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-3xl bg-white text-[#0f172a] rounded-lg shadow-lg p-4">
                <DialogHeader>
                ...
                <div className="mt-4 space-y-4">
                    <div>
                        ...
                        <div className="text-sm">{category.label} • {registered} registered {category.capacity ? `/ ${category.capacity}` : ""}</div>
                    </div>

                    {type === "rr+ko" && (
                        <div className="border rounded p-3 bg-slate-50">
                            <div className="text-sm font-medium mb-2">Round-robin pools</div>
                            <div className="grid grid-cols-2 gap-3">
                                {pools.map((pool, idx) => (
                                    <div key={idx} className="border rounded p-2 bg-white">
                                        <div className="text-sm font-semibold">Pool {idx + 1} ({pool.length})</div>
                                        ...
                                        <div className="text-xs text-slate-500 mt-2">Qualifiers from this pool: {rrQualPerPool}</div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="mt-4">
                                ...
                                <div className="text-xs text-slate-500">Total qualifiers: {totalQual} • recommended bracket: {koBracket} • byes: {byes}</div>
                    ...
                    {type === "league" && (
                        <div className="border rounded p-3 bg-slate-50">
                            <div className="text-sm font-medium mb-2">League (Round-robin) preview</div>
                            <div className="text-xs text-slate-500 mb-2">Pools: {rrPools} • teams per pool (approx): {Math.ceil(registered / Math.max(1, rrPools))}</div>
                    ...
                    {type === "knockout" && (
                        <div className="border rounded p-3 bg-slate-50">
                            ...
                            <div className="text-xs text-slate-500">Bracket size: {koBracket} • seeding: {koSeedMethod} • byes: {byes}</div>

                            <div className="mt-3 border rounded p-2 bg-white">
                                <div className="text-sm font-semibold">Round of {koBracket}</div>
                                <ul className="mt-2 space-y-1">
                                    {pairs.map((p, i) => <li key={i} className="text-sm">{p[0]} <span className="text-xs text-slate-400">vs</span> {p[1]}</li>)}
                                </ul>
                            </div>
                        </div>
                    )}
